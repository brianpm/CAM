...
subroutine kessler_register()
   ...
   call cnst_add('RAINQM', mwh2o, cpair, 0._r8, ixrain,                      &
                 longname='Grid box averaged rain water amount')

end subroutine kessler_register

!========================================================================================

subroutine kessler_init()
   ...
   ! mass mixing ratios.  *** Note that the outfld calls are made from diag_phys_writeout
   ...
   ! addfld/add_default calls for PRECL are made from diag_init
...
subroutine kessler_tend(state, ptend, ztodt)

   ! Run Kessler physics (see kessler.F90)
   ...
   real(r8) :: precl(pcols)     ! total precip from cloud sedimentation
