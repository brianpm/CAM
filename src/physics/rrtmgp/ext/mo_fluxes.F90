...
module mo_fluxes
  ...
  contains
    procedure :: reduce => reduce_base
    procedure :: are_desired => are_desired_base
  end type ty_fluxes
...
contains
  ...
  !
  function reduce_base(this, gpt_flux_up, gpt_flux_dn, k_dist, top_at_1, gpt_flux_dn_dir) result(error_msg)
...
!++dbg
        write(idx_str, '(4i5)') size(this%flux_up, 1), size(this%flux_up, 2), ncol, nlev
        error_msg = 'reduce: flux_up array incorrectly sized: '//trim(idx_str)
!--dbg
        return
      end if 
    end if 
    ...
    if(associated(this%flux_dn)) then 
        ...
        ! If direct downward flux is provided but not requested as output, add it to the
        !   total downwelling.
        if(present(gpt_flux_dn_dir) .and. .not. associated(this%flux_dn_dir)) & 
          this%flux_dn = this%flux_dn + sum(gpt_flux_dn_dir, DIM=3)
    end if
    ...
    if(associated(this%flux_net)) then
      ...
      if(associated(this%flux_dn) .and. associated(this%flux_up)) then 
        ...
        ! If direct flux isn't broken out in output we'll include it in the net
        if(present(gpt_flux_dn_dir) .and. .not. associated(this%flux_dn_dir)) &  
          this%flux_net = this%flux_net + sum(gpt_flux_dn_dir, dim = 3) 
      end if 
    end if 
