...
contains
  ! --------------------------------------------------------------------------------------
  function reduce_byband(this, gpt_flux_up, gpt_flux_dn, k_dist, top_at_1, gpt_flux_dn_dir) result(error_msg)
    ...
    ! 
    error_msg = this%ty_fluxes%reduce(gpt_flux_up, gpt_flux_dn, k_dist, top_at_1, gpt_flux_dn_dir)
    if(error_msg /= '') return
    ...
    if(associated(this%bnd_flux_up)) then
      do bnd = 1, nbnd
        gpt_lim(:) = k_dist%convert_band2gpt(bnd)
    ...
    if(associated(this%bnd_flux_dn)) then
      ...
      if(present(gpt_flux_dn_dir) .and. .not. associated(this%bnd_flux_dn_dir)) then  
        ! If direct downward flux is provided but not requested as output, add it to the
        !   total downwelling.
        do bnd = 1, nbnd
          gpt_lim(:) = k_dist%convert_band2gpt(bnd)
          this%bnd_flux_dn(:,:,bnd) = this%bnd_flux_dn(:,:,bnd) + & 
                                      sum(gpt_flux_dn_dir(:,:,gpt_lim(1):gpt_lim(2)), dim=3)
        end do  
      end if 
    end if
    ...
    if(associated(this%bnd_flux_net)) then
      ...
      if(associated(this%bnd_flux_dn) .and. associated(this%bnd_flux_up)) then 
        ...
        if(present(gpt_flux_dn_dir) .and. .not. associated(this%bnd_flux_dn_dir)) then  
          ...
          do bnd = 1, nbnd
            gpt_lim(:) = k_dist%convert_band2gpt(bnd)
            this%bnd_flux_net(:,:,bnd) = this%bnd_flux_net(:,:,bnd) + & 
                                         sum(gpt_flux_dn_dir(:,:,gpt_lim(1):gpt_lim(2)), dim=3)
          end do  
        end if 
      end if 
