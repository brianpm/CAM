...
! methods to be called from this layer.
!
!-----------------------------------------------------------------------
...
use dyn_comp,         only: dyn_import_t, dyn_export_t ! dummy types
use scanslt,          only: advection_state, scanslt_initial, scanslt_final
use prognostics,      only: ps, u3, v3, t3, q3, qminus, div, &
                            pdeld, dpsl, dpsm, omga, phis, n3, n3m2, n3m1
...
use time_manager,     only: is_first_step, is_nstep, get_step_size
...
subroutine stepon_init(dyn_in, dyn_out)
   ...
   ! Arguments
   type(dyn_import_t) :: dyn_in             ! included for interface compatibility
   type(dyn_export_t) :: dyn_out            ! included for interface compatibility

   ! Local variables
   integer :: k, lat, i
   !----------------------------------------------------------------------------

   call t_startf('stepon_init')
...
subroutine stepon_d_p(phys_state, phys_tend , pbuf2d, dyn_in, dyn_out, ztodt_out)
   ...
   real(r8), intent(out)              :: ztodt_out   ! twice time step unless nstep=0
   ...
   ! The first time stepon_d_p is called is during the initialization phase.
   ! is_first_step returns true during initialization (nstep=0)
...
subroutine stepon_run(phys_state, dyn_in, dyn_out, cam_out)
  ...
  if (eul_nsplit==1) then	
     ...
     dt_dyn = dt_dyn0
     if (is_nstep(1)) dt_dyn = 2*dt_dyn0
